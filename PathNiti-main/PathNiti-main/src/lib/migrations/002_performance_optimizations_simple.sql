...
-- This migration adds essential indexes and optimizations that work with standard Supabase

-- Additional indexes for colleges table
CREATE INDEX IF NOT EXISTS idx_colleges_location_state ON public.colleges USING btree ((location->>'state'));
CREATE INDEX IF NOT EXISTS idx_colleges_location_city ON public.colleges USING btree ((location->>'city'));
...
-- Create function to get college profile with optimized queries (simplified version)
...
-- Create function for paginated applications with better performance (simplified)
...
BEGIN
    ...
    -- Build base query for applications with student profile data
    ...
    count_query := 'SELECT COUNT(*) FROM public.student_applications sa WHERE sa.college_id = $1';
    
    -- Add status filter
    IF filter_status IS NOT NULL AND filter_status != 'all' THEN
        data_query := data_query || ' AND sa.status = $2';
        count_query := count_query || ' AND sa.status = $2';
    END IF;
    
    -- Add search filter (simple ILIKE search)
    IF search_term IS NOT NULL AND search_term != '' THEN
        IF filter_status IS NOT NULL AND filter_status != 'all' THEN
            data_query := data_query || ' AND (sa.full_name ILIKE $3 OR sa.email ILIKE $3 OR sa.phone ILIKE $3)';
            count_query := count_query || ' AND (sa.full_name ILIKE $3 OR sa.email ILIKE $3 OR sa.phone ILIKE $3)';
        ELSE
            data_query := data_query || ' AND (sa.full_name ILIKE $2 OR sa.email ILIKE $2 OR sa.phone ILIKE $2)';
            count_query := count_query || ' AND (sa.full_name ILIKE $2 OR sa.email ILIKE $2 OR sa.phone ILIKE $2)';
        END IF;
    END IF;
    
    -- Add pagination
    IF filter_status IS NOT NULL AND filter_status != 'all' AND search_term IS NOT NULL AND search_term != '' THEN
        data_query := data_query || ' LIMIT $4 OFFSET $5';
    ...
    ELSIF filter_status IS NOT NULL AND filter_status != 'all' THEN
        data_query := data_query || ' LIMIT $3 OFFSET $4';
    ...
    ELSE
        data_query := data_query || ' LIMIT $2 OFFSET $3';
